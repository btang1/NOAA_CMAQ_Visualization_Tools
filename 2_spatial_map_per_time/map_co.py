...
pattern = 'Surface_Map_co.nc'                                         #change chemical species
filename = dirw+ pattern
f = Dataset(filename,'r')

Lat_0  = f.variables['lat'][:]
Lon_0  = f.variables['lon'][:]
data_0 = f.variables['co'][:]                                         #change chemical species
...
for i in range(len(data_0)):
    ...
    m.pcolormesh(Lon_0, Lat_0, data_1,latlon=True, vmin = 0, vmax = 500, cmap = 'jet') 
    
    cb =plt.colorbar(orientation="horizontal")
    cb.set_label('CO (ppbv)',fontsize = 30)                                                            #change units
