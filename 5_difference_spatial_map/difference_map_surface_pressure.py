...
data_54 = f.variables['ps'][:]                                         #change chemical species
...
# Lat_0  = f.variables['lat'][:]
# Lon_0  = f.variables['lon'][:]
data_52 = f.variables['ps'][:]                                         #change chemical species
...
for i in range(len(data_54)):
    ...
    m.pcolormesh(Lon_0, Lat_0, data_1,latlon=True,vmin = -100, vmax = 100, cmap = 'bwr') 
    ...
    m.drawcountries()
    m.drawstates()
    plt.title('Differnce Map for Surface Pressure CMAQ5.4_develop vs. CMAQ5.2_develop \n@'+str(Time_CMAQ[i]),fontsize=30)
    plt.savefig('diffmap_ps_'+str(i),dpi = 100)
