...
data_54 = f.variables['pm25'][:]                                         #change chemical species
...
# Lat_0  = f.variables['lat'][:]
# Lon_0  = f.variables['lon'][:]
data_52 = f.variables['pm25'][:]                                         #change chemical species
...
for i in range(len(data_54)):
    ...
    m.drawcountries()
    m.drawstates()
    plt.title('Differnce Map for $PM_{2.5}$ CMAQ5.4_develop vs. CMAQ5.2_develop \n@'+str(Time_CMAQ[i]),fontsize=30)
    plt.savefig('diffmap_pm25_'+str(i),dpi = 100)
