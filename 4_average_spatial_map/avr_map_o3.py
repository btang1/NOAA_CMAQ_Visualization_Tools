...
m.pcolormesh(Lon_0, Lat_0, data_avr,latlon=True, vmin = 10, vmax = 90, cmap = 'jet')  #change chemical species range

cb =plt.colorbar(orientation="horizontal")
cb.set_label('$O_3$ (ppbv)',fontsize = 30)                                        #change chemical species                                                          #change units
cb.ax.tick_params(labelsize=30)

m.drawcoastlines(linewidth=1)
m.drawcountries()
m.drawstates()
plt.title('Averages for $O_3$ from '+ str(Time_CMAQ[0])+'\nto '+ str(Time_CMAQ[-1]),fontsize=30) #change chemical species
plt.savefig('avr_o3',dpi = 100)                                               #change chemical species
