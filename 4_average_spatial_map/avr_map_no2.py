...
m.pcolormesh(Lon_0, Lat_0, data_avr,latlon=True, vmin = 0, vmax = 10, cmap = 'jet')  #change chemical species range

cb =plt.colorbar(orientation="horizontal")
cb.set_label('$NO_2$ (ppbv)',fontsize = 30)                                        #change chemical species                                                          #change units
cb.ax.tick_params(labelsize=30)

m.drawcoastlines(linewidth=1)
m.drawcountries()
m.drawstates()
plt.title('Averages for $NO_2$ from '+ str(Time_CMAQ[0])+'\nto '+ str(Time_CMAQ[-1]),fontsize=30) #change chemical species
plt.savefig('avr_no2_',dpi = 100)                                               #change chemical species
